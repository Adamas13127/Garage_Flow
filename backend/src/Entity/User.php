...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    public function getCreatedUnavailabilities(): Collection
    {
        return $this->createdUnavailabilities;
    }

    public function addCreatedUnavailability(Unavailability $unavailability): static
    {
        if (!$this->createdUnavailabilities->contains($unavailability)) {
            $this->createdUnavailabilities->add($unavailability);
    ...
    {
        if ($this->createdUnavailabilities->removeElement($unavailability) && $unavailability->getCreatedBy() === $this) {
    ...
    {
        if ($this->vehicles->removeElement($vehicle) && $vehicle->getClient() === $this) {
    ...
    {
        if ($this->appointments->removeElement($appointment) && $appointment->getClient() === $this) {
    ...
    public function getInterventionStatusHistories(): Collection
    {
        return $this->interventionStatusHistories;
    }

    public function addInterventionStatusHistory(InterventionStatusHistory $history): static
    {
        if (!$this->interventionStatusHistories->contains($history)) {
            $this->interventionStatusHistories->add($history);
    ...
    {
        if ($this->interventionStatusHistories->removeElement($history) && $history->getChangedBy() === $this) {
    ...
    public function addInternalNote(InternalNote $note): static
    {
        if (!$this->internalNotes->contains($note)) {
    ...
    {
        if ($this->internalNotes->removeElement($note) && $note->getAuthor() === $this) {
            $note->setAuthor(null);
        }
