 ...
 * Il existe pour afficher un rendez-vous client et son action d'annulation si possible.
...
interface AppointmentCardProps { appointment: Appointment; onCancel?: () => void; }
...
export function AppointmentCard({ appointment, onCancel }: AppointmentCardProps) {
  ...
  return <AppCard title={appointment.garage?.nom ?? `Rendez-vous #${appointment.id}`} subtitle={formatDateTime(appointment.dateDebut)}><Text>{appointment.service?.nom ?? appointment.prestation?.nom ?? 'Prestation non renseignee'}</Text><Text>{appointment.commentaireClient ?? 'Aucun commentaire'}</Text><StatusBadge status={appointment.statut} />{canCancel && onCancel ? <AppButton variant="secondary" onPress={confirmCancel}>Annuler</AppButton> : null}</AppCard>;
}